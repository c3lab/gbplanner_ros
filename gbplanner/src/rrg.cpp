...
Rrg::Rrg(rclcpp::Node* node) : node_(node) {
  map_manager_ = new MapManager(node_);

  adaptive_obb_ = new AdaptiveObb(node_, map_manager_);

  initializeAttributes();
}

Rrg::Rrg(rclcpp::Node* node, MapManager* map_manager)
...
void Rrg::initializeAttributes() {
  visualization_ = new Visualization(node_);
  ...
  rostime_start_ = node_->now();
  //
  add_frontiers_to_global_graph_ = false;
  //
  exploring_direction_ = 0.0;
  periodic_timer_ = node_->create_wall_timer(
      std::chrono::duration<double>(kTimerPeriod),
      std::bind(&Rrg::timerCallback, this));
  ...
  pci_reset_pub_ = node_->create_publisher<std_msgs::msg::Bool>(
      "planner_control_interface/msg/reset", 10);

  local_free_map_pub_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>(
      "gbplanner/local_free_cloud", 10);

  path_pub_ = node_->create_publisher<nav_msgs::msg::Path>("/gbplanner_path", 10);

  free_cloud_pub_ = node_->create_publisher<sensor_msgs::msg::PointCloud2>(
      "freespace_pointcloud", 10);

  entry_point_pub_ = node_->create_publisher<geometry_msgs::msg::PoseStamped>(
      "gbplanner/entry_point_viz", 10);
  ...
  global_graph_update_timer_ = node_->create_wall_timer(
      ...
      std::bind(&Rrg::expandGlobalGraphTimerCallback, this));

  global_graph_frontier_addition_timer_ = node_->create_wall_timer(
      std::chrono::duration<double>(kGlobalGraphFrontierAdditionTimerPeriod),
      std::bind(&Rrg::expandGlobalGraphFrontierAdditionTimerCallback, this));

  camera_annotation_timer_ = node_->create_wall_timer(
      std::chrono::duration<double>(0.1),
      std::bind(&Rrg::cameraAnnotationTimerCallback, this));

  semantics_subscriber_ =
      node_->create_subscription<planner_semantic_msgs::msg::SemanticPoint>(
          "semantic_location", 100,
          [this](const planner_semantic_msgs::msg::SemanticPoint& msg) {
            semanticsCallback(msg);
          });

  stop_srv_subscriber_ = node_->create_subscription<std_msgs::msg::Bool>(
      "planner_control_interface/stop_request", 100,
      [this](const std_msgs::msg::Bool& msg) { stopMsgCallback(msg); });
  opening_detection_sub_ =
      node_->create_subscription<planner_msgs::msg::MultipleOpeningDetections>(
          "opening_detections", 100,
          [this](const planner_msgs::msg::MultipleOpeningDetections& msg) {
            openingDetectionCallback(msg);
          });
  query_pt_sub_ = node_->create_subscription<geometry_msgs::msg::PoseStamped>(
      "query_point", 1,
      [this](const geometry_msgs::msg::PoseStamped& msg) { queryPtCallback(msg); });
  cam_pitch_sub_ = node_->create_subscription<sensor_msgs::msg::JointState>(
      "cam_pitch", 1,
      [this](const sensor_msgs::msg::JointState& msg) { camPitchCallback(msg); });

  time_log_pub_ = node_->create_publisher<std_msgs::msg::Float32MultiArray>(
      "gbp_time_log", 10);

  pci_homing_ = node_->create_client<std_srvs::srv::Trigger>(
      "planner_control_interface/std_srvs/homing_trigger");
  landing_srv_client_ = node_->create_client<std_srvs::srv::Empty>(
      "land_srv");  // The service name should be remapped in the launch file

  reset_timer_srv_ = node_->create_service<std_srvs::srv::Trigger>(
      "gbplanner/reset_timer",
      std::bind(&Rrg::resetTimerCallback, this, std::placeholders::_1,
                std::placeholders::_2));

  pass_opening_srv_ = node_->create_service<std_srvs::srv::Trigger>(
      "get_opening_traversal_path",
      std::bind(&Rrg::getOpeningPathCallback, this, std::placeholders::_1,
                std::placeholders::_2));
  approve_passing_srv_ =
      node_->create_service<planner_msgs::srv::PlannerOpeningApproval>(
          "approve_opening_traversal",
          std::bind(&Rrg::approvePassingCallback, this, std::placeholders::_1,
                    std::placeholders::_2));
  ...
  query_srv_ = node_->create_service<std_srvs::srv::Trigger>(
      "query_srv", std::bind(&Rrg::queryCallback, this, std::placeholders::_1,
                             std::placeholders::_2));
  remove_geofence_srv_ =
      node_->create_service<planner_msgs::srv::PlannerSetPlanningMode>(
          "remove_geofence",
          std::bind(&Rrg::removeGeofenceCallback, this, std::placeholders::_1,
                    std::placeholders::_2));

  ele_map_sub_ = node_->create_subscription<grid_map_msgs::msg::GridMap>(
      "elevation_map", 1,
      [this](const grid_map_msgs::msg::GridMap& msg) { eleMapCallback(msg); });
...
void Rrg::reset() {
  ...
  if (add_frontiers_to_global_graph_) {
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Reset: Adding frontiers to global graph");
    add_frontiers_to_global_graph_ = false;
  ...
  if (VoxelStatus::kFree != voxel_state) {
    switch (voxel_state) {
      case VoxelStatus::kFree:
        RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Current box is Free.");
        break;
      case VoxelStatus::kOccupied:
        RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Current box contains Occupied voxels.");
        break;
      case VoxelStatus::kUnknown:
        RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Current box contains Unknown voxels.");
        break;
    }
    // Assume that even it is not fully free, but safe to clear these voxels.
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Starting position is not clear--> clear space around the robot.");
...
void Rrg::resetMapCallback(const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
                        ...
                        std::shared_ptr<std_srvs::srv::Trigger::Response> res)
{
  ...
  std::vector<geometry_msgs::msg::Pose> out_path = connectTSPOrderWithSubvertices(tsp_order, path_rep_map, graph);
  nav_msgs::msg::Path out_path_vis;
...
void Rrg::openingDetectionCallback(const planner_msgs::msg::MultipleOpeningDetections &detections) {
...
void Rrg::stopMsgCallback(const std_msgs::msg::Bool& msg) {
  global_exploration_ongoing_ = false;
  auto_global_planner_trig_ = false;
  opening_traversal_mode_ = OpeningTraversalMode::kNone;
}

void Rrg::camPitchCallback(const sensor_msgs::msg::JointState &state)
{
  cam_pitch_ = state.position[0];
}

void Rrg::queryPtCallback(const geometry_msgs::msg::PoseStamped& pose)
{
...
void Rrg::expandGraphEdges(std::shared_ptr<GraphManager> graph_manager,
  ...
  for (int i = 0; i < nearest_vertices.size(); ++i) {
    ...
    direction << nearest_vertices[i]->state[0] - origin[0],
        ...
        nearest_vertices[i]->state[2] - origin[2];
    double d_norm = direction.norm();
    // RCLCPP_WARN(node_->get_logger(), "Expand edges: d_norm: %f", d_norm);
    if ((d_norm > planning_params_.edge_length_min) &&
        (d_norm < planning_params_.edge_length_max)) {
      // RCLCPP_WARN(node_->get_logger(), "Expand edges: d_norm: %f: Within limits", d_norm);
      Eigen::Vector3d p_overshoot =
          direction / d_norm * planning_params_.edge_overshoot;
      Eigen::Vector3d p_start =
          origin + robot_params_.center_offset - p_overshoot;
...
Rrg::GraphStatus Rrg::batchGraph(){
  ...
  if (global_exploration_ongoing_) {
    ...
    if ((current_state_.head(3) - global_vertex->state.head(3)).norm() > 5.0) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Global frontier not reached. Triggering global planner again");
      local_exploration_ongoing_ = false;
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Formed a graph with [%d] vertices and [%d] edges",
  ...
  if (local_graph_->getNumVertices() > 1) {
    ...
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Number of failed samples: [%d] vertices and [%d] edges",
...
Rrg::GraphStatus Rrg::buildGraph() {
  ...
  planner_trigger_count_++;
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Formed a graph with [%d] vertices and [%d] edges with [%d] loops",
...
Rrg::GraphStatus Rrg::buildGridGraph(StateVec state, Eigen::Vector3d robot_size,
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Number of nodes [%d][%d][%d].", num_nodes[0], num_nodes[1],
           num_nodes[2]);
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Initialized matrices.");
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Number of free nodes [%d].", free_vertex_count);
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Free edges: [%d]", free_edge_count);
  ...
  // Add source vertex.
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Grid graph: %d vertices, %d edges", local_graph_->getNumVertices(),
...
Rrg::GraphStatus Rrg::evaluateGraph() {
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Evaluate graph]: Gain Calculation Done");
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Mean path len timer: %f", path_len_time / path_len_count);
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Mean path dir timer: %f", path_dir_time / path_dir_count);
  ...
  if (planning_params_.auto_global_planner_enable) {
    if (!frontier_exists || best_gain <= 0) {
      ++num_low_gain_iters_;
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "No frontier found in this round. Total rounds: %d",
               num_low_gain_iters_);
    } else {
      if (num_low_gain_iters_ > 0) --num_low_gain_iters_;
    }
    if (num_low_gain_iters_ >= planning_params_.max_num_low_gain_iters) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "%d consecutinve low gain paths, triggering global planner.",
               num_low_gain_iters_);
      num_low_gain_iters_ = 0;
      auto_global_planner_trig_ = true;
  ...
  dt = tc.endTimer();
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "No frontier check timer: %f", dt);
  ...
  dt = tc.endTimer();
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Visualization timer: %f", dt);
  ...
  if (best_gain > 0) {
    ...
    //
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Best path: with gain [%f] and ID [%d] ", best_gain, best_path_id);
  ...
  dt = tc.endTimer();
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Best vertex selection timer: %f", dt);
...
Rrg::LocalPlannerStatus Rrg::evaluateLocalNavigationPath()
{
  ...
  if(local_navigation_goal_set_)
  {
    ...
    geometry_msgs::msg::PointStamped local_target_msg;
    local_target_msg.header.frame_id = world_frame_;
    local_target_msg.header.stamp = node_->now();
...
bool Rrg::modifyPath(pcl::PointCloud<pcl::PointXYZ>* obstacle_pcl,
  ...
  if ((dist_min_sq == std::numeric_limits<double>::max()) ||
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[IMPRV] Path too close to obstacle");
    return false;
  }
...
void Rrg::addFrontiers(int best_vertex_id) {
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Global graph: %d vertices, %d edges.",
  ...
  if (update_global_frontiers) {
    ...
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Have %d frontiers from global graph.",
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Got %d leaf vertices from newly local graph.",
           (int)leaf_vertices.size());
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Got %d frontiers from newly local graph.",
...
void Rrg::resetTimerCallback(const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
                             std::shared_ptr<std_srvs::srv::Trigger::Response> res) {
    (void)req;
    resetMissionTimer();
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::PLANNER_STATUS, "Mission time reset.");
    res->success = true;
  }
...
void Rrg::expandGlobalGraphFrontierAdditionTimerCallback() {
  if (add_frontiers_to_global_graph_) {
    ...
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Timer: Adding frontiers to global graph");
    add_frontiers_to_global_graph_ = false;
    addFrontiers(0);  // id given as 0 because it is not used
    
    t2 = std::chrono::high_resolution_clock::now();
    double chrono_time = std::chrono::duration<double, std::milli>(t2 - t1).count();
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Add frontiers time: %f", chrono_time);
  }
}

void Rrg::expandGlobalGraphTimerCallback() {
  // Algorithm:
  ...
  t2 = std::chrono::high_resolution_clock::now();
  double chrono_time = std::chrono::duration<double, std::milli>(t2 - t1).count();
  // RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Expand global graph time: %f", chrono_time);
}

void Rrg::semanticsCallback(
    const planner_semantic_msgs::msg::SemanticPoint& semantic) {
  ...
  if (VoxelStatus::kFree !=
      map_manager_->getBoxStatus(sem + robot_params_.center_offset,
                                 robot_box_size_, true)) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[SEMANTICS]: Marker state not free.");
    return;
  }

  Vertex* temp_nearest_vertex;
  if (!global_graph_->getNearestVertex(&new_state, &temp_nearest_vertex)) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[SEMANTICS]: No nearest vertex found.");
    return;
  }
  std::vector<Vertex*> nearest_vertices;
  ...
  for (int i = 0; (i < nearest_vertices.size()) && (i < kMaxNumTrials); ++i) {
    ...
    geometry_msgs::msg::Pose start, end;
  ...
  if (!path_status) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[SEMANTICS]: Cannot connect to nearest node");
    return;
  } else {
    std::vector<Vertex*> semantic_path;
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, 
...
bool Rrg::search(geometry_msgs::msg::Pose source_pose,
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Start searching ...");
  int final_target_id;
...
ConnectStatus Rrg::findPathToConnect(
    ...
    int& final_target_id, std::vector<geometry_msgs::msg::Pose>& path_ret) {
  ConnectStatus status;
  path_ret.clear();
  graph_manager->reset();

  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Search a path from src [%f,%f,%f] to tgt [%f,%f,%f]", source[0],
  ...
  if (try_straight_path) {
    ...
    if (voxel_state == VoxelStatus::kFree) {
      RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Try straight path...");
  ...
  if (params.check_collision_at_source) {
    ...
    if (VoxelStatus::kFree != voxel_state) {
      switch (voxel_state) {
        case VoxelStatus::kOccupied:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Source position contains Occupied voxels --> Stop.");
          break;
        case VoxelStatus::kUnknown:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Source position contains Unknown voxels  --> Stop.");
          break;
        case VoxelStatus::kFree:
          break;
      }
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Built a graph with %d vertices and %d edges.",
  ...
  if (reached_target) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Reached target.");
    ...
    if (voxel_state == VoxelStatus::kFree) {
      ...
      if (rep.status == ExpandGraphStatus::kSuccess) {
        RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Added target to the graph successfully.");
        num_vertices += rep.num_vertices_added;
        num_edges += rep.num_edges_added;
        added_target = true;
        target_vertex = rep.vertex_added;
      } else {
        RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Cannot expand the graph to connect to the target.");
      }
    } else {
      RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Target is not free, failed to add to the graph.");
    }
  } else {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "ConnectStatus::kErrorNoFeasiblePath");
  ...
  if (!added_target) {
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Sorting best path.");
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Get shortest path [%d] from %d path.", target_vertex->id,
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Finish searching.");
  status = ConnectStatus::kSuccess;
...
bool Rrg::loadParams(bool shared_params) {
  ...
  // through other code (ex. behaviour planner).
  // ROS 2 parameters are node-local, so the "<node name>/" prefix the ROS 1
  // code built here disappears; the remaining separator is '.'.

  // Load all relevant parameters.
  if (!sensor_params_.loadParams(node_, "SensorParams")) return false;

  if (!planning_params_.loadParams(node_, "PlanningParams")) return false;
  ...
  // }
  camera_annotation_params_.loadParams(node_, "CameraAnnotationParams");

  if (!free_frustum_params_.loadParams(node_, "FreeFrustumParams")) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), param_verbosity >= Verbosity::WARN, "No setting for FreeFrustumParams.");
    planning_params_.freespace_cloud_enable = false;
  }

  if(!planning_params_.yaw_tangent_correction) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), param_verbosity >= Verbosity::ERROR, "[PlanningParams] yaw_tangent_correction is false, disabling path_safety_enhance_enable.");
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "NoGainZones size: %d", planning_params_.no_gain_zones_list.size());
  ...
  if (planning_params_.no_gain_zones_list.size() <= 0) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "No NoGainZones.");
    use_no_gain_space_ = false;
  } else {
    for (auto& zone : planning_params_.no_gain_zones_list) {
      BoundedSpaceParams ngz;
      if (!ngz.loadParams(node_, "NoGainZones." + zone)) {
        continue;
      }
  ...
  if (!shared_params) {
    if (!robot_params_.loadParams(node_, "RobotParams")) return false;
    if (!global_space_params_.loadParams(node_, "BoundedSpaceParams.Global"))
      return false;
  }

  if (!local_space_params_.loadParams(node_, "BoundedSpaceParams.Local"))
    return false;

  if (!local_search_params_.loadParams(node_, "BoundedSpaceParams.LocalSearch"))
    return false;
  if (!local_adaptive_params_.loadParams(
          node_, "BoundedSpaceParams.LocalAdaptiveExp")) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "No setting for adaptive exploration mode.");
  }
  if (!adaptive_obb_->loadParams("AdaptiveObbParams")) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "No setting for adaptive exploration mode.");
  }
  ...
  if (!random_sampler_.loadParams(node_,
                                  "RandomSamplerParams.SamplerForExploration"))
    return false;
  if (!random_sampler_to_search_.loadParams(
          node_, "RandomSamplerParams.SamplerForSearching"))
    return false;
  if (!random_sampler_adaptive_.loadParams(
          node_, "RandomSamplerParams.SamplerForAdaptiveExp")) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "No setting for adaptive exploration mode.");
  }

  if (!robot_dynamics_params_.loadParams(node_, "RobotDynamics")) return false;

  if (!geofence_manager_->loadParams(node_, "GeofenceParams")) return false;
  if (!darpa_gate_params_.loadParams(node_, "DarpaGateParams")) return false;
...
bool Rrg::setGlobalBound(planner_msgs::msg::PlanningBound& bound,
  ...
  if (!reset_to_default) {
    ...
    if ((current_state_.x() + robot_params_.center_offset.x() <
      ...
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, 
...
bool Rrg::setGlobalBound(
  ...
  if (bound.reset_to_default) {
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "world frame: %s, %s", world_frame_.c_str(), planning_params_.global_frame_id.c_str());
    // Transform points to the world frame
    // ROS 1 left the transform default-constructed (uninitialised Bullet
    // storage) when the lookup threw; identity keeps the failure path defined.
    tf2::Transform darpa_to_world_transform;
    darpa_to_world_transform.setIdentity();
    try {
      tf2::fromMsg(tf_buffer_
                       ->lookupTransform(world_frame_, bound.header.frame_id,
                                         tf2::TimePointZero)
                       .transform,
                   darpa_to_world_transform);
    } catch (const tf2::TransformException& ex) {
      RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "%s", ex.what());
    }
    // Center
    tf2::Vector3 center_tf;
    tf2::fromMsg(bound.center, center_tf);
    tf2::Vector3 center_trans_tf = darpa_to_world_transform * center_tf;
    geometry_msgs::msg::Point center_trans_point;
    tf2::toMsg(center_trans_tf, center_trans_point);
    Eigen::Vector3d center;
    convertPointToEigen(center_trans_point, center);
    // Left
    tf2::Vector3 left_tf;
    tf2::fromMsg(bound.left, left_tf);
    tf2::Vector3 left_trans_tf = darpa_to_world_transform * left_tf;
    geometry_msgs::msg::Point left_trans_point;
    tf2::toMsg(left_trans_tf, left_trans_point);
    Eigen::Vector3d left;
    convertPointToEigen(left_trans_point, left);
    // Up
    tf2::Vector3 up_tf;
    tf2::fromMsg(bound.up, up_tf);
    tf2::Vector3 up_trans_tf = darpa_to_world_transform * up_tf;
    geometry_msgs::msg::Point up_trans_point;
    tf2::toMsg(up_trans_tf, up_trans_point);
    Eigen::Vector3d up;
    convertPointToEigen(up_trans_point, up);
    // Front
    tf2::Vector3 front_tf;
    tf2::fromMsg(bound.front, front_tf);
    tf2::Vector3 front_trans_tf = darpa_to_world_transform * front_tf;
    geometry_msgs::msg::Point front_trans_point;
    tf2::toMsg(front_trans_tf, front_trans_point);
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Visualization done");
  return true;
}

void Rrg::getGlobalBound(planner_msgs::msg::PlanningBound& bound) {
...
void Rrg::computeExplorationGain(bool only_leaf_vertices, bool clustering) {
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Num clusters: %d", num_clusters);
  stat_->compute_exp_gain_time = GET_ELAPSED_TIME(tim);
  t2 = std::chrono::high_resolution_clock::now();
  stat_chrono_->compute_exp_gain_time =
      std::chrono::duration<double, std::milli>(t2 - t1).count();
...
void Rrg::computeVolumetricGainRayModel(StateVec& state, VolumetricGain& vgain,
  ...
  for (int ind = 0; ind < planning_params_.exp_sensor_list.size(); ++ind) {
    ...
    // RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Total number of voxels queried: %d", (num_unknown_voxels +
      ...
      if (global_space_params_.isInsideSpace(voxel)) {
        ...
        if (no_gain_zone_cleared) {
          ...
          if (vs == VoxelStatus::kUnknown) {
            ...
            RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "Unsupported voxel type.");
          }
          if (vis_en) voxel_log.push_back(std::make_pair(voxel, vs));
        }
      }
...
void Rrg::setRootStateForPlanning(const geometry_msgs::msg::Pose& root_pose) {
  ...
  state_for_planning_[3] = tf2::getYaw(root_pose.orientation);
...
bool Rrg::setHomingPos() {
  if (global_graph_->getNumVertices() == 0) {
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Global graph is empty: add current state as homing position.");
    Vertex* g_root_vertex =
        new Vertex(global_graph_->generateVertexID(), current_state_);
    global_graph_->addVertex(g_root_vertex);
    return true;
  } else {
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Global graph is not empty, can not set current state as homing.");
    return false;
  }
}

std::vector<geometry_msgs::msg::Pose> Rrg::searchHomingPath(
  ...
  if (global_graph_->getNumVertices() <= 1) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[GlobalGraph] Graph is empty, nothing to search for homing.");
    return ret_path;
  }
  ...
  if (direction_norm <= kRadiusLimit) {
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[GlobalGraph] Try to add current state to the graph.");
    ExpandGraphReport rep;
    expandGraph(global_graph_, cur_state, rep);
    if (rep.status == ExpandGraphStatus::kSuccess) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[GlobalGraph] Added successfully.");
      ...
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[GlobalGraph] Can not add current state to graph since: ");
      switch (rep.status) {
        case ExpandGraphStatus::kErrorKdTree:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "kErrorKdTree.");
          break;
        case ExpandGraphStatus::kErrorCollisionEdge:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "kErrorCollisionEdge.");
          break;
        case ExpandGraphStatus::kErrorShortEdge:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "kErrorShortEdge.");
          break;
        default:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "kErrorUnknown.");
          break;
      }
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[GlobalGraph] Failed to find global path.");
    }
  }

  if (connect_state_to_graph) {
    if (!global_graph_->findShortestPaths(global_graph_rep_)) {
      RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "[GlobalGraph] Failed to find shortest path.");
      return ret_path;
    }
    ...
    if (homing_path_id.empty() || homing_path_id.back() != 0) {
      RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "[GlobalGraph] Could not find a path to home.");
      return ret_path;
    }
    ...
    bool is_similar = comparePathWithDirectionApprioximately(
        ret_path, tf2::getYaw(ret_path[0].orientation));
...
std::vector<geometry_msgs::msg::Pose> Rrg::getGlobalPath(
  ...
  if (!global_graph_->getNearestVertex(&wp, &wp_nearest_vertex)) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Cannot find any nearby vertex to reposition.");
    return ret_path;
  ...
  if (direction_norm <= kRadiusLimit) {
    ...
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ...
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[GlobalGraph] Failed to find global path.");
    }
  }

  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Finding a path from current[%d] to vertex[%d].", link_vertex->id,
  ...
  if (planning_params_.path_safety_enhance_enable) {
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Compute an aternate path for homing in %f(s)", dmod_time);
  ...
  std::vector<geometry_msgs::msg::Pose> interp_path;
...
void Rrg::getTargetCosts(const std::vector<geometry_msgs::msg::Point>& targets,
                         std::vector<planner_msgs::msg::TargetCost>& costs) {
  costs.clear();
  ...
  // with the request.
  planner_msgs::msg::TargetCost unreachable;
  unreachable.reachable = false;
  unreachable.mode = planner_msgs::msg::TargetCost::MODE_DIRECT;
  ...
  if (num_vertices <= 1) {
    ...
    for (const auto& t : targets) {
      planner_msgs::msg::TargetCost c = unreachable;
      c.target = t;
      costs.push_back(c);
    }
    return;
  ...
  if (!global_graph_->findShortestPaths(robot_vertex->id, from_robot)) {
    RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR,
  ...
  for (const auto& t : targets) {
    ...
    if (global_graph_->getNearestVertexInRange(
      ...
      c.mode = planner_msgs::msg::TargetCost::MODE_DIRECT;
      c.path_length =
    ...
    if (!needs_frontier) {
      ...
      c.mode = planner_msgs::msg::TargetCost::MODE_LOCAL;
      c.path_length = 0.0;
    ...
    for (const auto& leg : frontier_legs) {
      ...
      if (cost < best_cost) {
        best_cost = cost;
        c.mode = planner_msgs::msg::TargetCost::MODE_VIA_FRONTIER;
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG,
...
void Rrg::getFrontiers(bool skip_gain_refresh,
                       std::vector<planner_msgs::msg::FrontierPoint>& frontiers) {
  frontiers.clear();
  int num_vertices = global_graph_->getNumVertices();
  if (num_vertices <= 1) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN,
  ...
  for (auto& f : global_frontiers) {
    ...
    planner_msgs::msg::FrontierPoint fp;
    fp.id = f->id;
  ...
  std::sort(frontiers.begin(), frontiers.end(),
            [](const planner_msgs::msg::FrontierPoint& a,
               const planner_msgs::msg::FrontierPoint& b) {
...
bool Rrg::reconnectPathBlindly(std::vector<geometry_msgs::msg::Pose>& ref_path,
                               std::vector<geometry_msgs::msg::Pose>& mod_path) {
  ...
  double prev_yaw = tf2::getYaw(mod_path[0].orientation);
...
bool Rrg::homingRequired(std::vector<geometry_msgs::msg::Pose> &homing_path)
{
  homing_path.clear();
  double time_elapsed = 0.0;
  if (node_->now().seconds() != 0.0) {
    if (rostime_start_.seconds() == 0.0) rostime_start_ = node_->now();
    time_elapsed = (double)((node_->now() - rostime_start_).seconds());
  }
  ...
  if (!homing_path.empty()) {
    ...
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Time to home: %f; Time remaining: %f", time_to_home,
              time_remaining);

    const double kTimeDelta = 20;
    if (time_to_home > time_remaining - kTimeDelta) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::PLANNER_STATUS, "REACHED TIME LIMIT: HOMING ENGAGED.");
      if (planning_params_.path_safety_enhance_enable) {
        std::vector<geometry_msgs::msg::Pose> mod_path;
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Can not find a path to return home from here.");
    return false;  // Homing might be needed but can't find path
  }
  return false;
}

std::vector<geometry_msgs::msg::Pose> Rrg::getBestPathSimplified()
{
  std::vector<geometry_msgs::msg::Pose> ret, empty_path;
  ...
  if(!planning_params_.allow_sudden_dir_change)
  {
    ...
    {
      if (!res) {
        RCLCPP_WARN(node_->get_logger(), "Changing exploration direction.[%f --> %f]", exploring_direction_,
                best_path_direction);
        ++dir_change_count_;
        return ret;
      }
  ...
  for (int i = 0; i < best_path.size(); ++i) {
    ...
    if ((dir_vec.norm() > 0) &&
      ...
      RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Segment [%d] is not clear.", i);
    }
    ...
    quat.setZ(q_eigen.z());
    quat.setW(q_eigen.w());
    tf2::Vector3 origin(best_path[i][0], best_path[i][1], best_path[i][2]);
    tf2::Transform poseTF(quat, origin);
    geometry_msgs::msg::Pose pose;
    tf2::toMsg(poseTF, pose);
    ret.push_back(pose);
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Best path:  size = %d, length = %f, time = %f", (int)ret.size(),
           traverse_length, traverse_time);
  // Put this into global graph for homing later.
  bool path_added = false;
  if ((int)ret.size() <= 1) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "No ground attached path");
    return empty_path;
  }
  ...
  if (planning_params_.path_safety_enhance_enable) {
    ...
    double dmod_time = GET_ELAPSED_TIME(mod_time);
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Compute an aternate path in %f(s)", dmod_time);
    visualization_->visualizeModPath(mod_path);
  }
...
std::vector<geometry_msgs::msg::Pose> Rrg::getBestPath(std::string tgt_frame,
                                                  int& status) {
  ...
  if (planning_params_.auto_landing_enable &&
    ...
    if (time_budget_remaining <= 0.0) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::PLANNER_STATUS, "RAN OUT OF TIME BUDGET --> LANDING.");
      landing_engaged_ = true;
      std_msgs::msg::Bool stop_msg;
      stop_msg.data = true;

      // Fire and forget: the ROS 1 call discarded the response, and blocking
      // on it from inside a callback would deadlock a single-threaded executor.
      if (!landing_engaged_) {
        landing_srv_client_->async_send_request(
            std::make_shared<std_srvs::srv::Empty::Request>(),
            // The empty completion callback lets rclcpp retire the pending
            // request; without one the entry is never reaped.
      ...
      status = planner_msgs::srv::PlannerSrv::Response::FORWARD;
      return empty_path;
    }
  }
  // Decide if need to go home.
  if (planning_params_.auto_homing_enable) {
    status = planner_msgs::srv::PlannerSrv::Response::HOMING;
    std::vector<geometry_msgs::msg::Pose> homing_path;
    double time_elapsed = 0.0;
    if (node_->now().seconds() != 0.0) {
      if (rostime_start_.seconds() == 0.0) rostime_start_ = node_->now();
      time_elapsed = (double)((node_->now() - rostime_start_).seconds());
    ...
    double time_budget_remaining =
        planning_params_.time_budget_limit - time_elapsed;
    if (time_budget_remaining <= 0.0) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::PLANNER_STATUS, "RAN OUT OF TIME BUDGET --> STOP HERE.");
      return homing_path;
    }
    if (current_battery_time_remaining_ <= 0.0) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::PLANNER_STATUS, "RAN OUT OF BATTERY --> STOP HERE.");
      return homing_path;
    }
    // Check two conditions whatever which one comes first.
    double time_remaining =
  ...
  if (total_len <= len_min_thres) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Best path is too short.");
    return ret;
  }
...
bool Rrg::improveFreePath(const std::vector<geometry_msgs::msg::Pose>& path_orig,
                          std::vector<geometry_msgs::msg::Pose>& path_mod,
                          bool relaxed) {
  ...
  // Interpolate path
  std::vector<geometry_msgs::msg::Pose> path_orig_interp = path_orig;
  double kInterpolationDistance =
      planning_params_.path_interpolation_distance * 2.0;
  std::vector<geometry_msgs::msg::Pose> interp_path_orig;
  ...
  std::vector<geometry_msgs::msg::Pose> path_mod1 = path_orig_interp;
  ...
  while (cont_refine) {
    ...
    if (path_mod1.size() > 2) {
      for (int i = 0; i < (path_mod1.size() - 2); ++i) {
        ...
        // RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Segment length %f.", segment_len);
        if (segment_len < kSegmentLenMin) {
          if ((VoxelStatus::kFree ==
            ...
            RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Combine nodes to remove short segments.");
  ...
  for (int i = 1; i < path_mod1.size(); ++i) {
    ...
    if (e1_admissible) {
      ...
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Segment not free");
      seg_free = false;
    }
    ...
    if (seg_free && modification_successful) {
      ...
      if (!(e1_admissible) ||
        ...
        p1_mod = p1;
        mod_success = false;
        RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Newly modified path is not collision-free.");
...
void Rrg::generateGridSamplesBasic(std::vector<int> &viewpoint_ids) {
  ...
  // RCLCPP_WARN(node_->get_logger(), "Centroid: %f, %f, %f", centroid.x(), centroid.y(), centroid.z());
  ...
  // RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Number of ")
...
std::vector<geometry_msgs::msg::Pose> Rrg::getInspectionPathBasic() {
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Inspection bounds:");
  if(global_verbosity >= Verbosity::DEBUG) {
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Num of vertices in conneting graph: %d", local_graph_->getNumVertices());
  
  std::vector<int> viewpoint_ids;
  generateGridSamplesBasic(viewpoint_ids);  
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Total Num of vertices graph: %d", local_graph_->getNumVertices());

  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Number of Viewpoints: %d", viewpoint_ids.size());
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Inpsection]: Graph building: %f", graph_timer.endTimer());
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Centroid: %f, %f, %f", centroid.x(), centroid.y(), centroid.z());
  ...
  for(int i : viewpoint_ids) {
    ...
    if(true) {
      ...
      // RCLCPP_INFO(node_->get_logger(), "Vertex: %d, voxles seen: %d", v->id, v->vol_gain.unseen_voxel_hash_keys.size());
      cumulative_unseen_voxels.merge(v->vol_gain.unseen_voxel_hash_keys);
    }
  }
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Inpsection]: Gain calculation: %f", gain_timer.endTimer());
  ...
  while(current_coverage <= min_coverage && !remaining_vertices.empty()) {
    ...
    // RCLCPP_INFO(node_->get_logger(), "Best order: ");
    ...
    for (int i = 0; i < remaining_vertices.size(); ++i)
    {
      ...
      // }
      // RCLCPP_INFO(node_->get_logger(), "Vertex: %d", i);
      int visibility_before = remaining_vertices[i].second.size();
      // RCLCPP_INFO(node_->get_logger(), "Before: %d", remaining_vertices[i].second.size());
      ...
      //   RCLCPP_INFO(node_->get_logger(), "Before: %d", visibility_before);
      //   RCLCPP_INFO(node_->get_logger(), "After: %d", remaining_vertices[i].second.size());
      // }
    ...
    if(selected_vertices.size() > planning_params_.max_inspection_vertices) {
      RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Max inspection vertices limit reached");
      break;
    }
  }

  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Selected vertices: %d, total vertices: %d", selected_vertices.size(), local_graph_->getNumVertices());
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Achieved coverage: %d, Max possible coverage: %d, percentage: %f", current_coverage, cumulative_unseen_voxels.size(), (100.0*current_coverage)/cumulative_unseen_voxels.size());

  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Inpsection]: Sorting: %f", sorting_timer.endTimer());
  ...
  if(!selected_vertices.empty()) {
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Inpsection]: TSP: Shortest path calculations: %f", T1.endTimer());
    // T1.reset();
    // visualization_->visualizeGraphVertices(local_graph_, viewpoint_ids);
    // RCLCPP_WARN(node_->get_logger(), "[Inpsection]: TSP: Visualization: %f", T1.endTimer());

    // Connecting the selected viewpoints
    T1.reset();
    std::vector<std::vector<int>> cost_matrix;
		generateCostMatrix(selected_vertex_ids, cost_matrix, path_rep_map);
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Inpsection]: TSP: Cost matrix: %f", T1.endTimer());
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Inpsection]: TSP: doTSP: %f", T1.endTimer());
    ...
    if(tsp_order[0] == 0) {
      ...
      if(it == tsp_order.end()) {
        RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "Root vertex not in TSP order");
      }
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Inpsection]: TSP: connectTSPOrder: %f", T1.endTimer());
    ...
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "No vertex selected");
  }
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "[Inpsection]: TSP: %f", tsp_timer.endTimer());

  t2 = std::chrono::high_resolution_clock::now();

  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "Inspection path calculation time: %f", std::chrono::duration<double, std::milli>(t2 - t1).count());
...
std::vector<geometry_msgs::msg::Pose> Rrg::getInspectionPath() {
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Num of vertices in conneting graph: %d", local_graph_->getNumVertices());
  
  std::vector<int> viewpoint_ids;
  generateGridSamples(viewpoint_ids);  
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Total Num of vertices graph: %d", local_graph_->getNumVertices());

  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Number of Viewpoints: %d", viewpoint_ids.size());
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[Inpsection]: Graph building: %f", graph_timer.endTimer());
  ...
  Eigen::Vector3d centroid = (inspection_bound_.max_val + inspection_bound_.min_val) / 2.0;
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Centroid: %f, %f, %f", centroid.x(), centroid.y(), centroid.z());
  ...
  for(int i : viewpoint_ids) {
    ...
    for(auto subv : v->orientation_sub_vertices)
    {
      ...
      // RCLCPP_INFO(node_->get_logger(), "Vertex: %d, subvertex: %d, voxles seen: %d", v->id, subv->id, subv->vol_gain.unseen_voxel_hash_keys.size());
      cumulative_unseen_voxels.merge(subv->vol_gain.unseen_voxel_hash_keys);
    }

  }
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[Inpsection]: Gain calculation: %f", gain_timer.endTimer());
  ...
  while(current_coverage <= min_coverage && !remaining_vertices.empty()) {
    ...
    for (int i = 0; i < remaining_vertices.size(); ++i)
    {
      ...
      // RCLCPP_INFO(node_->get_logger(), "Before: %d", std::get<2>(remaining_vertices[i]).size());
    ...
    if(selected_vertices.size() > planning_params_.max_inspection_vertices) {
      RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Max inspection vertices limit reached");
      break;
    }
  }
...
std::vector<geometry_msgs::msg::Pose> Rrg::connectTSPOrder(std::vector<int> &tsp_nodes, std::map<int, ShortestPathsReport> &path_rep_map, std::shared_ptr<GraphManager> graph)
{
  // std::cout << std::endl;
	std::vector<geometry_msgs::msg::Pose> tsp_path;
	for (int i = 0; i < tsp_nodes.size() - 1; ++i)
	{
		std::vector<StateVec> current_path_segment_vec;
		std::vector<geometry_msgs::msg::Pose> current_path_segment;
...
std::vector<geometry_msgs::msg::Pose> Rrg::connectTSPOrderWithSubvertices(std::vector<std::pair<int, std::vector<int>>> &tsp_nodes, std::map<int, ShortestPathsReport> &path_rep_map, std::shared_ptr<GraphManager> graph)
{
  std::vector<geometry_msgs::msg::Pose> tsp_path;
...
bool Rrg::addRefPathToGraph(const std::shared_ptr<GraphManager> graph_manager,
  ...
  if (direction_norm <= kDeltaLimit) {
    ...
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ...
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[GlobalGraph] Can not add current state to the global graph.");
      return false;
    }
  }
  ...
  for (int i = 0; i < vertex_list.size(); ++i) {
    ...
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ...
      switch (rep.status) {
        case ExpandGraphStatus::kErrorKdTree:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Can not add this vertex: kErrorKdTree.");
          break;
        case ExpandGraphStatus::kErrorCollisionEdge:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Can not add this vertex: kErrorCollisionEdge.");
          break;
        case ExpandGraphStatus::kErrorShortEdge:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Can not add this vertex: kErrorShortEdge.");
          break;
      }
    }
  }
...
void Rrg::freePointCloudtimerCallback() {
  ...
  for (auto sensor_name : free_frustum_params_.sensor_list) {
    ...
    out_cloud.header.stamp = node_->now();
    free_cloud_pub_->publish(out_cloud);
  }

  auto t2 = std::chrono::high_resolution_clock::now();
  std::chrono::duration<double> elapsed = t2 - t1;
  // RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Free cloud time: %f s", elapsed.count());
}

void Rrg::timerCallback() {
  // Re-initialize until get non-zero value.
  // RCLCPP_WARN(node_->get_logger(), "tC: start: GG verts: %d, edges: %d", global_graph_->getNumVertices(), global_graph_->getNumEdges());
  if (rostime_start_.seconds() == 0) rostime_start_ = node_->now();
  TIMER tcbtime;
  START_TIMER(tcbtime);
  auto t1 = std::chrono::high_resolution_clock::now();
  auto t2 = t1;
  ...
  if (!odometry_ready) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Planner is waiting for odometry");
    return;
  }
  ...
  if (planning_params_.auto_landing_enable && !landing_engaged_ &&
    ...
    if (time_budget_remaining <= 0.0) {
      if (!local_exploration_ongoing_) {
        RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "RAN OUT OF TIME BUDGET --> LANDING.");
        landing_engaged_ = true;
        std_msgs::msg::Bool stop_msg;
        stop_msg.data = true;
        landing_srv_client_->async_send_request(
    ...
    while (robot_backtracking_queue_.size()) {
      ...
      if (robot_backtracking_prev_) {
        ...
        if (dir_norm >= kOdoEnforceLength) {
          ...
          ExpandGraphReport rep;
          // RCLCPP_WARN(node_->get_logger(), "Expand edges on odometry");
  ...
  // RCLCPP_WARN(node_->get_logger(), "tC: end: GG verts: %d, edges: %d", global_graph_->getNumVertices(), global_graph_->getNumEdges());
  // visualization_->visualizeGlobalGraph(global_graph_);
}
...
std::vector<int> Rrg::performShortestPathsClustering(
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Cluster %d paths into %d clusters.", (int)vertices.size(),
  ...
  if (refinement_enable) {
    ...
    RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Clustering with refinement %d paths into %d clusters.",
...
bool Rrg::connectStateToGraph(std::shared_ptr<GraphManager> graph,
  ...
  if (direction_norm <= kDelta) {
    ...
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ...
      switch (rep.status) {
        case ExpandGraphStatus::kErrorKdTree:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "kErrorKdTree.");
          break;
        case ExpandGraphStatus::kErrorCollisionEdge:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "kErrorCollisionEdge.");
          break;
        case ExpandGraphStatus::kErrorShortEdge:
          RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "kErrorShortEdge.");
          break;
      }
    }
  }
...
bool Rrg::isRemainingTimeSufficient(const double& time_cost,
  ...
  if (time_spare < kTimeDelta) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "REACHED TIME LIMIT: BE CAREFUL.");
    return false;
  }
  return true;
}

void Rrg::getOpeningPathCallback(const std::shared_ptr<std_srvs::srv::Trigger::Request> req,
                                 std::shared_ptr<std_srvs::srv::Trigger::Response> res) {
	(void)req;
	std::vector<geometry_msgs::msg::Pose> path = getOpeningTraversalPath();
...
void Rrg::approvePassingCallback(
    const std::shared_ptr<planner_msgs::srv::PlannerOpeningApproval::Request> req,
    std::shared_ptr<planner_msgs::srv::PlannerOpeningApproval::Response> res) {
  (void)res;
  if(req->approval == planner_msgs::srv::PlannerOpeningApproval::Request::APPROVED) {
    opening_passing_approved_ = OpeningApproval::kApproved;
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Opening Passing Approved");
  }
  else if(req->approval == planner_msgs::srv::PlannerOpeningApproval::Request::REJECTED) {
    opening_passing_approved_ = OpeningApproval::kRejected;
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Opening Passing Rejected");
  }
  else if(req->approval == planner_msgs::srv::PlannerOpeningApproval::Request::RE_EVALUATE) {
    opening_passing_approved_ = OpeningApproval::kReEvaluate;
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Opening Passing to be Re-evaluated");
  }
}
...
std::vector<geometry_msgs::msg::Pose> Rrg::getOpeningTraversalPath(OpeningTraversalMode mode, OpeningTraversalStatus &status) {
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "OPENING under exect: %d", opening_under_execution_);
  ...
  if(mode == OpeningTraversalMode::kPathCheck)
  {
    ...
    {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "This OPENING [%d] does not exist", opening_under_execution_);
      status = OpeningTraversalStatus::OPENING_DOUBLE_CHECK_FAILED;
      return through_path;
    }
  }
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Detections updated");


	if(detected_openings_.empty()) {
    RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "No Openings at all");
  ...
  if(!found) {
    RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "No Appropriate Opening Found");
    status = OpeningTraversalStatus::NO_OPENINGS;
    return through_path;
  }

  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Best opening found: %d", opening_under_execution_);
  ...
  double direction = tf2::getYaw(best_opening->pose.orientation);
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Path direction set");

  geometry_msgs::msg::Quaternion corrected_quat = best_opening->pose.orientation;
  ...
  if(getDistance(current_state_, p0) < getDistance(current_state_, p1)) {
      ...
      corrected_quat = tf2::toMsg(quat);
      p0.orientation = tf2::toMsg(quat);
      best_opening->pose.orientation = tf2::toMsg(quat);
      p1.orientation = tf2::toMsg(quat);
  }
  else {
    tf2::Quaternion quat;
  ...
  tf2::Quaternion corrected_quat_tf;
  tf2::fromMsg(corrected_quat, corrected_quat_tf);
  double corrected_yaw = tf2::getYaw(corrected_quat_tf);

  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Corrected quat: %f", corrected_yaw);
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Path set 1");
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Path set 2");
  ...
  if(mode != OpeningTraversalMode::kPassingThrough)
  {
    ...
    if(success) {
      ...
      RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::PLANNER_STATUS, "Connecting path not found");
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Path set 3");

  // RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "OPENING Mode (even later): %d", opening_traversal_mode_);
...
std::vector<geometry_msgs::msg::Pose> Rrg::getOpeningTraversalPath() {
	std::vector<geometry_msgs::msg::Pose> through_path;


  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "OPENING Mode: %d", opening_traversal_mode_);
  ...
  if(opening_traversal_mode_ == OpeningTraversalMode::kNone) {
    ...
    if(opening_passing_approved_ == OpeningApproval::kApproved) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Opening Traversal Approved");
      opening_passing_approved_ = OpeningApproval::kWaiting;
      opening_traversal_mode_ = OpeningTraversalMode::kPassingThrough;
    }
    else if(opening_passing_approved_ == OpeningApproval::kRejected) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Opening Traversal Rejected");
      opening_traversal_mode_ = OpeningTraversalMode::kNone;
      detected_openings_[opening_under_execution_]->active = false;
      return through_path;
    }
    else if(opening_passing_approved_ == OpeningApproval::kReEvaluate) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Opening Traversal ReEvaluate");
      opening_traversal_mode_ = OpeningTraversalMode::kPathCheck;
    }
  }
  ...
  if(!found) {
    RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "No Opening Found");
    opening_traversal_mode_ = OpeningTraversalMode::kNone;
    return through_path;
  }
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Path direction set");
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "OPENING Mode (later): %d", opening_traversal_mode_);
...
std::vector<geometry_msgs::msg::Pose> Rrg::calculateGlobalPath()
{
  visualization_->visualizeGlobalGraph(global_graph_);
  std::vector<geometry_msgs::msg::Pose> ret_path;
  ret_path.clear();
  ...
  if (global_graph_->getNumVertices() <= 1) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[GlobalGraph] Graph is empty, nothing to search.");
    return ret_path;
  }
  ...
  std::vector<Vertex*> global_frontiers;
  int num_vertices = global_graph_->getNumVertices();
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Re-check all frontiers.");
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Currently have %d frontiers in the global graph.",
           (int)global_frontiers.size());
  if ((global_frontiers.size() <= 0)) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::PLANNER_STATUS, "No frontier exists");
    return ret_path;
  }

  // Let's try to add current state to the global graph.
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Trying to add new vertex from current position.");
  StateVec cur_state;
  cur_state << current_state_[0], current_state_[1], current_state_[2],
      current_state_[3], current_state_[4];
  cur_state[2] -=
  ...
  if (!connected_to_graph) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Cannot add the state to the global graph.");
    return ret_path;
  }
  ...
  // Find best frontier
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[Global Planner] Auto mode");
  ...
  RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::DEBUG, "Get %d feasible frontiers from global frontiers.",
  ...
  std::vector<int> frontier_to_home_path_id;
  if (best_gain >= 0) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Found the best frontier to go is: %d", best_frontier->id);
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "runGlobalPlanner costs: %f (s)", dtime);
...
std::vector<geometry_msgs::msg::Pose> Rrg::reRunGlobalPlanner(int &status) {
  return runGlobalPlanner(current_global_vertex_id_, true, true, status);
}

std::vector<geometry_msgs::msg::Pose> Rrg::runGlobalPlanner(int vertex_id,
  ...
  status = planner_msgs::srv::PlannerSrv::Response::REPOSITIONING;
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "1");
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "2");
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "3");

  // Check if the time endurance is still available.
  if (!ignore_time) {
    if (getTimeRemained() <= 0.0) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[Global] RAN OUT OF TIME --> STOP HERE.");
      return ret_path;
    }
  }

  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "4");
  ...
  if ((!not_check_frontier) && (global_frontiers.size() <= 0)) {
    ...
    if (planning_params_.go_home_if_fully_explored) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::PLANNER_STATUS, " --> Calling HOMING instead.");
  ...
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "5");
  ...
  if (vertex_id) {
    // Manual mode
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[Global Planner] Manual Mode");
    ...
    double time_cost = 0;
    if (!ignore_time) {
      RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "[Global] Time remaining: %f (sec)", time_remaining);
      time_cost += time_to_target;
      RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "[Global] Time to [%3d]: %f (sec)", vertex_id, time_to_target);
      if (planning_params_.auto_homing_enable) {
        time_cost += time_to_home;
        RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "[Global] Time to home  : %f (sec)", time_to_home);
      }
    }
    double time_spare = 0;
    if (!isRemainingTimeSufficient(time_cost, time_spare)) {
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "[Global] Not enough time to go the vertex [%d]", vertex_id);
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, 
    ...
    // Print out
    // RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "List of potential frontier in decreasing order of gain:");
    // for (int i = 0; i < feasible_global_frontiers.size(); ++i) {
    //   RCLCPP_INFO_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::INFO, "ID [%d]: %d with gain %f", i,
...
void Rrg::removeGeofenceCallback(
    const std::shared_ptr<planner_msgs::srv::PlannerSetPlanningMode::Request> req,
    std::shared_ptr<planner_msgs::srv::PlannerSetPlanningMode::Response> res)
{
  (void)res;
...
void Rrg::eleMapCallback(const grid_map_msgs::msg::GridMap& msg)
{
  grid_map::GridMapRosConverter::fromMessage(msg, ele_map_);
}


void Rrg::addGeofenceAreas(const geometry_msgs::msg::PolygonStamped& polygon_msgs) {
  if ((planning_params_.geofence_checking_enable)) {
    ...
    if (!polygon_msgs.header.frame_id.compare(
      ...
      try {
        // The member buffer replaces the throw-away listener the ROS 1 code
        // created here; lookupTransform's own timeout subsumes
        // waitForTransform.
        tf2::fromMsg(tf_buffer_
                         ->lookupTransform(planning_params_.global_frame_id,
                                           polygon_msgs.header.frame_id,
                                           tf2::TimePointZero,
                                           tf2::durationFromSec(0.1))
                         .transform,
                     tf_to_global);
        for (int i = 0; i < polygon_msgs.polygon.points.size(); ++i) {
          tf2::Vector3 poly_in_global;
...
void Rrg::clearUntraversableZones() {
  geofence_manager_.reset(new GeofenceManager());
  RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Clear all the traversable polygons.");
}
...
bool Rrg::isPathCollisionFree(const std::vector<geometry_msgs::msg::Pose>& path,
...
std::vector<geometry_msgs::msg::Pose> Rrg::searchPathToPassGate() {
  std::vector<geometry_msgs::msg::Pose> ret_path;

  // Check if this is allowed.
  if (!darpa_gate_params_.enable) {
    RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Not allow to run search to pass the gate");
    return ret_path;
  }
  ...
  if (darpa_gate_params_.load_from_darpa_frame) {
    ...
    try {
      tf2::fromMsg(tf_buffer_
                       ->lookupTransform(darpa_gate_params_.world_frame_id,
                                         darpa_gate_params_.gate_center_frame_id,
                                         tf2::TimePointZero,
                                         tf2::durationFromSec(1.0))
                       .transform,
                   tfW2G);
      tf2::Vector3 vec = tfW2G.getOrigin();
      ...
      // gate_heading = tf2::getYaw(tfW2G.getRotation());
      if(global_verbosity >= Verbosity::INFO) {
        std::cout << "Darpa gate center: " << gate_center << std::endl;
      }
    } catch (const tf2::TransformException& ex) {
      RCLCPP_ERROR_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::ERROR, "%s", ex.what());
      RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Could not look up TF for gate center.");
      return ret_path;
    }
  }
  ...
  if (ret_path.empty()) RCLCPP_WARN_EXPRESSION(node_->get_logger(), global_verbosity >= Verbosity::WARN, "Could not find path to go through.");
  return ret_path;
}

void Rrg::cameraAnnotationTimerCallback() {
  if(!planning_params_.annotate_map_with_camera) return;

  // RCLCPP_WARN(node_->get_logger(), "Camera annotation enabled");
