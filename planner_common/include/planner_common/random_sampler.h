...
class RandomSamplerBase {
 public:
  enum RandomDistributionType {
    ...
    kNormalUniform  // Normally and Uniformly sample, have to define a bound.
  };
  ...
  void setPDF(RandomSamplerBase::RandomDistributionType pdf_type);
  void setSTD(double scl_factor);
  RandomSamplerBase::RandomDistributionType getInitPDF();
  RandomDistributionType getPDF();
  double getRange();
  double getZOffset();  
  void reset();
...
class RandomSampler {
 public:
  ...
  void setPDF(RandomSamplerBase::RandomDistributionType pdf_type, int axis);
  ...
  void setSTD();
  std::vector<RandomSamplerBase::RandomDistributionType> getPDF();
  std::vector<RandomSamplerBase::RandomDistributionType> getInitPDF();
 ...
 private:
  ...
  const int kValidSamplesMaxSize = 2000;
  const int kInvalidSamplesMaxSize = 2000;
