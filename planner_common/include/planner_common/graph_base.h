...
#include "planner_semantic_msgs/msg/semantic_class.hpp"
...
struct SampleStatistic {
  ...
  void printTime() {
    total_time = build_graph_time + compute_exp_gain_time + shortest_path_time +
                 evaluate_graph_time;
    RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER,
...
struct BoundingBoxType {
  ...
  void set(geometry_msgs::msg::Point& p_min, geometry_msgs::msg::Point& p_max,
  ...
  void get(geometry_msgs::msg::Point& p_min, geometry_msgs::msg::Point& p_max) {
    p_min.x = min_val.x();
    p_min.y = min_val.y();
    p_min.z = min_val.z();
    p_max.x = max_val.x();
...
struct Vertex {
  Vertex(int v_id, StateVec v_state) {
    ...
    is_checked = false;
    dm = 0;
    semantic_class.value = planner_semantic_msgs::msg::SemanticClass::NONE;
  }
  ...
  bool is_checked;
  // Semantic
  planner_semantic_msgs::msg::SemanticClass semantic_class;
  // Edge map
