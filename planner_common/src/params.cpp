...
bool SensorParamsBase::loadParams(rclcpp::Node* node, const std::string& ns) {
  ...
  param_name = ns + ".type";
  getParamOpt(node, param_name, parse_str);
  ...
  if (!getParamOpt(node, param_name, min_range)) {
    min_range = 0.0;
    ROSPARAM_WARN(param_name, min_range);
  }

  param_name = ns + ".max_range";
  if (!getParamOpt(node, param_name, max_range)) {
    max_range = 5.0;
  ...
  if ((!getParamOpt(node, param_name, param_val)) || (param_val.size() != 3)) {
    param_val.resize(3);
    param_val[0] = 0.0;
    param_val[1] = 0.0;
    param_val[2] = 0.0;
  ...
  param_val.clear();
  param_name = ns + ".fov";
  if ((!getParamOpt(node, param_name, param_val)) || (param_val.size() != 2)) {
    param_val.resize(2);
    param_val[0] = 1.57;     // 90
  ...
  if (!getParamOpt(node, param_name, frontier_percentage_threshold)) {
    frontier_percentage_threshold = 0.1;
    ROSPARAM_WARN(param_name, frontier_percentage_threshold);
  }

  param_name = ns + ".sensor_frame";
  if (!getParamOpt(node, param_name, sensor_frame)) {
    sensor_frame = "";
    ROSPARAM_WARN(param_name, sensor_frame);
  }

  param_name = ns + ".frame_id";
  if (!getParamOpt(node, param_name, frame_id)) {
    frame_id = "";
    ROSPARAM_WARN(param_name, frame_id);
  }

  param_name = ns + ".width";
  if (!getParamOpt(node, param_name, width)) {
    width = 0;
    ROSPARAM_WARN(param_name, frame_id);
  }

  param_name = ns + ".height";
  if (!getParamOpt(node, param_name, height)) {
    height = 0;
    ROSPARAM_WARN(param_name, frame_id);
  }

  param_name = ns + ".width_removal";
  if (!getParamOpt(node, param_name, widthRemoval)) {
    widthRemoval = 0;
    ROSPARAM_WARN(param_name, frame_id);
  }

  param_name = ns + ".height_removal";
  if (!getParamOpt(node, param_name, heightRemoval)) {
    heightRemoval = 0;
    ROSPARAM_WARN(param_name, frame_id);
  }
  ...
  if (type == SensorType::kCamera) {
    if (width == 0 && height == 0) {
      ...
      height = h;
      RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO,
...
void SensorParamsBase::updateFrustumEndpoints() {
  rot_B2S = Eigen::AngleAxisd(rotations[0], Eigen::Vector3d::UnitZ()) *
            ...
            Eigen::AngleAxisd(rotations[2], Eigen::Vector3d::UnitX());
  rot_S2B = rot_B2S.inverse();
  // RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO, "[PARAMS]: fov: h: %f v: %f", fov[0], fov[1]);
  double v_res, h_res;
  v_res = (resolution[1] > 0) ? resolution[1] : (1.0 * M_PI / 180.0);
  h_res = (resolution[0] > 0) ? resolution[0] : (1.0 * M_PI / 180.0);
  if (type == SensorType::kCamera) {
...
bool SensorParams::loadParams(rclcpp::Node* node, const std::string& ns) {
  ROSPARAM_INFO("Loading: " + ns);
  std::string param_name;
  std::vector<double> param_val;
  ...
  getParamOpt(node, param_name, parse_str_list);
  ...
  for (auto it = sensor_list.begin(); it != sensor_list.end(); ++it) {
    ...
    if (spb.loadParams(node, sensor_ns)) {
...
bool RobotParams::loadParams(rclcpp::Node* node, const std::string& ns) {
  ROSPARAM_INFO("Loading: " + ns);
  std::string param_name;
  std::vector<double> param_val;
  ...
  if (!getParamOpt(node, param_name, relax_ratio)) {
    relax_ratio = 0.5;
...
void RobotParams::setBoundMode(BoundModeType bmode) {
  ...
  switch (bound_mode) {
    case BoundModeType::kExtendedBound:
      RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO, "Set bound mode: kExtendedBound");
      break;
    case BoundModeType::kRelaxedBound:
      RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO, "Set bound mode: kRelaxedBound");
      break;
    case BoundModeType::kMinBound:
      RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO, "Set bound mode: kMinBound");
      break;
    case BoundModeType::kExactBound:
      RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO, "Set bound mode: kExactBound");
      break;
    case BoundModeType::kNoBound:
      RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO, "Set bound mode: kNoBound");
      break;
  }
}
...
bool BoundedSpaceParams::loadParams(rclcpp::Node* node, const std::string& ns) {
  ROSPARAM_INFO("Loading: " + ns);
  std::string param_name;
  std::vector<double> param_val;
  ...
  if (type == BoundedSpaceType::kCuboid) {
    ...
    if (!getParamOpt(node, param_name, radius)) {
    ...
    if (!getParamOpt(node, param_name, radius_extension)) {
...
bool PlanningParams::loadParams(rclcpp::Node* node, const std::string& ns) {
  ROSPARAM_INFO("Loading: " + ns);
  std::string param_name;
  ...
  if (!getParamOpt(node, param_name, auto_global_planner_enable)) {
  ...
  if (!getParamOpt(node, param_name, relaxed_corridor_multiplier)) {
  ...
  if (!getParamOpt(node, param_name, interpolate_projection_distance)) {
  ...
  if (!getParamOpt(node, param_name, go_home_if_fully_explored)) {
  ...
  if (!getParamOpt(node, param_name, opening_traversal_path_edge_length)) {
  ...
  if (!getParamOpt(node, param_name, max_inspection_vertices)) {
  ...
  if (!getParamOpt(node, param_name, inspection_target_viewing_range)) {
  ...
  if (!getParamOpt(node, param_name, use_flipped_yaw)) {
    use_flipped_yaw = false;
    ROSPARAM_WARN(param_name, use_flipped_yaw);
  }

  param_name = ns + ".max_opening_height";
  if (!getParamOpt(node, param_name, max_opening_height)) {
  ...
  if (!getParamOpt(node, param_name, max_surface_distance)) {
  ...
  if (!getParamOpt(node, param_name, limit_vertices_to_surface)) {
  ...
  if (!getParamOpt(node, param_name, local_navigation_max_fail_iters)) {
  ...
  if ((!getParamOpt(node, param_name, param_val)) || (param_val.size() % 3 != 0)) {
    if(!exploration_only) {
      param_val.resize(3);
      param_val[0] = 0.0;
      param_val[1] = 0.0;
  ...
  compartment_dimensions.loadParams(node, param_name);
...
void PlanningParams::setPlanningMode(PlanningModeType pmode) {
  switch (pmode) {
    case PlanningModeType::kBasicExploration:
      type = PlanningModeType::kBasicExploration;
      RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Exploration mode is set to kBasicExploration");
      break;
    default:
      type = PlanningModeType::kBasicExploration;
      RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Exploration mode is set to kBasicExploration");
...
bool RobotDynamicsParams::loadParams(rclcpp::Node* node, const std::string& ns) {
  ROSPARAM_INFO("Loading: " + ns);
  std::string param_name;
...
bool DarpaGateParams::loadParams(rclcpp::Node* node, const std::string& ns) {
  ROSPARAM_INFO("Loading: " + ns);
  std::string param_name;
  std::vector<double> param_val;

  param_name = ns + ".enable";
  if (!getParamOpt(node, param_name, enable)) {
    ROSPARAM_WARN(param_name, enable);
  }

  param_name = ns + ".load_from_darpa_frame";
  if (!getParamOpt(node, param_name, load_from_darpa_frame)) {
    ROSPARAM_WARN(param_name, load_from_darpa_frame);
  }

  param_name = ns + ".world_frame_id";
  if (!getParamOpt(node, param_name, world_frame_id)) {
    ROSPARAM_WARN(param_name, world_frame_id);
  }

  param_name = ns + ".gate_center_frame_id";
  if (!getParamOpt(node, param_name, gate_center_frame_id)) {
