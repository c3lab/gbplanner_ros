...
Polygon2d::Polygon2d(const geometry_msgs::msg::Polygon& poly_msg) {
...
void GeofenceManager::addGeofenceArea(const geometry_msgs::msg::Polygon poly_msg,
                                      bool merge) {
  RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Added new geofence area.");
...
void GeofenceManager::addGeofenceArea(Polygon2d& poly, bool merge) {
  ...
  RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Number of geofence areas: %d", (int)geofence_list_.size());
}
      ...
      for (int j = i + 1; j < geofence_list_.size(); ++j) {
        if ((geofence_list_[i].polygon.doIntersect(
          ...
          if (union_poly.size() == 1) {
            ...
            RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Polygon with inner ring or they don't intersect.");
          }
        }
      }
      if (redo) break;
...
bool GeofenceManager::loadParams(rclcpp::Node* node, const std::string& ns) {
  ...
  if (!getParamOpt(node, param_name, geofence_init_list)) {
    // return true;
    ...
    for (auto& s : geofence_init_list) {
      ...
      if ((!getParamOpt(node, param_name, param_val)) ||
          (param_val.size() != 3)) {
