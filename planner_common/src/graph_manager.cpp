...
void GraphManager::removeEdge(Vertex* v, Vertex* u) {
  ...
  {
    RCLCPP_WARN(PLANNER_COMMON_LOGGER, "Edge between %d, %d does not exist in edge_map_", v->id, u->id);
  }
  // Remove from u's map
  auto u_it = std::find_if(edge_map_[u->id].begin(), edge_map_[u->id].end(),
                      [&v](const std::pair<int, double> a) {
  ...
  {
    RCLCPP_WARN(PLANNER_COMMON_LOGGER, "Edge between %d, %d does not exist in v neighbor map", v->id, u->id);
  }
  // Remove from u's map
  auto un_it = std::find_if(u->neighbors.begin(), u->neighbors.end(),
                      [&v](const int a) {
...
void GraphManager::getShortestPath(int target_id,
  ...
  path.clear();
  if (!rep.status) {
    RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Shortest paths report is not valid");
    return;
  }

  if (rep.parent_id_map.size() <= target_id) {
    RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Vertext with ID [%d] doesn't exist in the graph", target_id);
    return;
  }
  ...
  if (parent_id == target_id) {
    RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Vertex with ID [%d] is isolated from the graph", target_id);
    return;
  }
...
double GraphManager::getShortestDistance(int target_id,
  ...
  if (rep.parent_id_map.size() <= target_id) {
    RCLCPP_WARN_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::WARN, "Vertex with ID [%d] doesn't exist in the graph", target_id);
    return dist;
  }
...
void GraphManager::convertGraphToMsg(planner_msgs::msg::Graph& graph_msg) {
  ...
  for (auto& v : vertices_map_) {
    ...
    // tf2::Quaternion::setEuler takes (yaw, pitch, roll) in the same order as
    // the tf::Quaternion it replaces -- do not swap this for setRPY.
    tf2::Quaternion quat;
    quat.setEuler(0.0, v.second->state[4], v.second->state[3]);
    tf2::Vector3 origin(v.second->state[0], v.second->state[1], v.second->state[2]);
    tf2::Transform poseTF(quat, origin);
    // tf2 has no Transform -> Pose overload, so do what tf::poseTFToMsg did.
    tf2::toMsg(poseTF.getOrigin(), vertex.pose.position);
    vertex.pose.orientation = tf2::toMsg(poseTF.getRotation());
...
void GraphManager::saveGraph(const std::string& path) {
  ...
  // NOTE: this is CDR, not the ROS 1 wire format -- graph files are not
  // interchangeable between the Noetic and the Jazzy planner.
  rclcpp::Serialization<planner_msgs::msg::Graph> serializer;
  rclcpp::SerializedMessage serialized_msg;
  serializer.serialize_message(&graph_msg, &serialized_msg);
  const rcl_serialized_message_t& rcl_msg =
      serialized_msg.get_rcl_serialized_message();

  // Write to a file
  std::ofstream wrt_file(path, std::ios::out | std::ios::binary);
  wrt_file.write(reinterpret_cast<const char*>(rcl_msg.buffer),
                 static_cast<std::streamsize>(rcl_msg.buffer_length));
  wrt_file.close();
  RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO, "Save the graph with [%d] vertices and [%d] edges to a file: %s",
...
void GraphManager::loadGraph(const std::string& path) {
  ...
  rcl_serialized_message_t& rcl_msg =
      serialized_msg.get_rcl_serialized_message();
  pbuf->sgetn(reinterpret_cast<char*>(rcl_msg.buffer), size);
  rcl_msg.buffer_length = static_cast<size_t>(size);
  read_file.close();

  // Deserialize data into msg
  planner_msgs::msg::Graph graph_msg;
  rclcpp::Serialization<planner_msgs::msg::Graph> serializer;
  serializer.deserialize_message(&serialized_msg, &graph_msg);

  // Reconstruct the graph
  reset();
  convertMsgToGraph(graph_msg);
  RCLCPP_INFO_EXPRESSION(PLANNER_COMMON_LOGGER, global_verbosity >= Verbosity::INFO, "Load the graph with [%d] vertices and [%d] edges from a file: %s",
           getNumVertices(), getNumEdges(), path.c_str());
}
