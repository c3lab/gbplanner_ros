...
template <typename SDFServerType, typename SDFVoxelType>
double MapManagerVoxblox<SDFServerType, SDFVoxelType>::getResolution() {
  return sdf_layer_->voxel_size();
}

template <typename SDFServerType, typename SDFVoxelType>
bool MapManagerVoxblox<SDFServerType, SDFVoxelType>::getStatus() {
  return true;
}
...
bool MapManagerVoxblox<voxblox::EsdfServer, voxblox::EsdfVoxel>::
    checkUnknownStatus(const voxblox::EsdfVoxel* voxel) {
...
template <typename SDFServerType, typename SDFVoxelType>
VoxelStatus MapManagerVoxblox<SDFServerType, SDFVoxelType>::getVoxelStatus(
...
template <typename SDFServerType, typename SDFVoxelType>
VoxelStatus MapManagerVoxblox<SDFServerType, SDFVoxelType>::getRayStatus(
...
VoxelStatus MapManagerVoxblox<SDFServerType, SDFVoxelType>::getRayStatus(
  ...
  // std::cout << " [getRayStatus]: ray len: " << ray_len << std::endl;
  for (double d = 0; d < ray_len; d += voxel_size) {
    ...
    // std::cout << "  Voxel " << voxel_coordi.transpose();
  ...
  //   SDFVoxelType* voxel = sdf_layer_->getVoxelPtrByGlobalIndex(global_index);
  //   voxblox::Point voxel_center = voxblox::getCenterPointFromGridIndex(
  //       global_index, voxel_size_inv);
  //   if (checkUnknownStatus(voxel)) {
  //     if (stop_at_unknown_voxel) {
  //       end_voxel = voxel_center.cast<double>();
  //       tsdf_dist = -tsdf_integrator_config_.default_truncation_distance;
  //       return VoxelStatus::kUnknown;
  //     }
  //   } else if (voxel->distance <= distance_thres) {
...
double MapManagerVoxblox<SDFServerType, SDFVoxelType>::getPointDistance(
  ...
  if (!success) {
    ...
    if (!success)
      ...
      if (out_dist < 0.0) out_dist = 0.01;  // Occupied
    }
  } else {
    if (out_dist < 0.0) out_dist = 0.01;  // Occupied
  }
