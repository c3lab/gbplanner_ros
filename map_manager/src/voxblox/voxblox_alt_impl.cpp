...
VoxelStatus MapManagerVoxblox<SDFServerType, SDFVoxelType>::getBoxStatus(
    const Eigen::Vector3d& center, const Eigen::Vector3d& size,
    bool stop_at_unknown_voxel) {
  //
  if(box_status_method_ == 1) {    
...
template <typename SDFServerType, typename SDFVoxelType>
VoxelStatus MapManagerVoxblox<SDFServerType, SDFVoxelType>::getPathStatus(
    const Eigen::Vector3d& start, const Eigen::Vector3d& end,
    const Eigen::Vector3d& box_size, bool stop_at_unknown_voxel) {
  //
  if(line_status_method_ == 1) {
    float voxel_size = sdf_layer_->voxel_size();
    float voxel_size_inv = 1.0 / voxel_size;
    ...
    //   // Eigen::Vector3d vox_center = start_e + i * edge_vec.normalized() / (check_count-1);
    //   Eigen::Vector3d vox_center = start_e + i * edge_vec.normalized();
